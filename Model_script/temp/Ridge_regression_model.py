...
def main(df) :
    ...
    model = linear_model.Ridge(alpha=100000) #alpha value was determined using model_selection.cross_val_score from the Python package scikit-learn
    ...
    with open(folder_path+'_model.pickle', mode = 'wb') as f: #Saving model
        pickle.dump(model, f)
    with open(folder_path+'_model.columns', mode = 'wb') as f: #Saving features_list
...
if __name__ == "__main__":
    df=pd.read_csv(sys.argv[1],delimiter="\t") #log10 conversion were conducted in CSsite values
    main(df)
